...
import os
import datetime
from cassiopeia import Match, Role
...
def get_positions(timeline: Timeline):
    ...
    for i in range(1, 11):
        ...
        for k in frames[i].participant_frames:
            participant_id = frames[i].participant_frames[k].participant_id
            position = None

            # Position on the map
            coord = [frames[i].participant_frames[k].position.x,
                     frames[i].participant_frames[k].position.y]
...
def get_stats_at_10(timeline: Timeline):
    participant_frame = timeline.frames[10].participant_frames
    stats_at_10 = {}
    for k in participant_frame:
        participant_id = participant_frame[k].participant_id
        minions_killed = participant_frame[k].creep_score
        jungle_minions_killed = participant_frame[k].neutral_minions_killed
        row = {
            "minionsKilled": minions_killed,
            "jungleMinionsKilled": jungle_minions_killed,
            "jungleMinionRatio": jungle_minions_killed / (
                        minions_killed + jungle_minions_killed) if minions_killed > 0 else 0
        }
...
def get_features(match: Match, timeline: Timeline):
    ...
    for participant in match.participants:
        ...
        for i in participant.stats.items:
            # Check if there is an item for the slot
            if i is not None:
                for role in role_composition:
                    item_id = str(i.id)

                    if item_id in overUsedItems[role]:
                        participant_features["has-item-overUsed-" + role] = 1

                    if item_id in mostlyUsedItems[role]:
                        participant_features["has-item-mostlyUsed-" + role] = 1

                    if item_id in underUsedItems[role]:
                        participant_features["has-item-underUsed-" + role] = 1

        # Summoner spells
        participant_features.update(spells)
        participant_features["spell-" + str(participant.summoner_spell_d.id)] = 1
        participant_features["spell-" + str(participant.summoner_spell_f.id)] = 1
...
def predict(match: Match, timeline: Timeline):
    if match.duration < datetime.timedelta(minutes=12):
